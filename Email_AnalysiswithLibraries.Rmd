...
sent_to<-c("Rahman", "Driver", "Slinger")
sent_to[3]
class(sent_to)
...
for (i in 1:length(sent_to)){
        ...
        if (i==1){ 
                ...
                commonWords <- input_data %>% mostcommon( x=50)
                commonBigrams <- input_data %>% mostcommon(n=2, x=50) %>% cbind(sent_to[i])
        }
        else {
                input_data<-individual_emails(raw_data, who=sent_to[i])
                commonWords <- input_data %>% mostcommon( x=50) %>% rbind(commonWords)
                commonBigrams <- input_data %>% mostcommon(n=2, x=50) %>% cbind(sent_to[i]) %>% rbind(commonBigrams)
        }
}
